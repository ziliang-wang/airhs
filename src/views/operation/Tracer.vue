<template>
  <div class="tracer-container" id="16" ref="tracerPageRef" v-loading="loading">
    ...
    <!-- 依起飞资讯 -->
    <!-- <el-form :inline="true" :model="form1" :rules="form1Rules" ref="form1Ref">
      <el-tag style="margin-bottom: 20px;">依起飛資訊</el-tag>
      <br>
      <el-form-item label="主單號" prop="mawbNo">
        <el-input v-model.trim="form1.mawbNo" placeholder="主單號"></el-input>
      </el-form-item>
      <el-form-item label="起飛資訊" prop="flight">
        <el-input v-model.trim="form1.flight" placeholder="如: CI 6986/28"></el-input>
      </el-form-item>
      <el-form-item>
        <span v-if="isEnableQuery">
          <el-button type="primary" @click="queryFlightInfo" class="queryBtn">查詢</el-button>
        </span>
        <span v-else>
          <el-button disabled title="您沒有查詢權限" type="primary" @click="queryFlightInfo" class="queryBtn">查詢</el-button>
        </span>
      </el-form-item>
    </el-form>
    <el-divider></el-divider> -->
    ...
    <!-- <el-form :inline="true" :model="form2" :rules="form2Rules" ref="form2Ref">
      <el-tag style="margin-bottom: 20px;">依未飛重量</el-tag>
      <br>
      <el-form-item label="主單號" prop="mawbNo">
        <el-input v-model.trim="form2.mawbNo" placeholder="主單號"></el-input>
      </el-form-item>
      <el-form-item label="未飛重量" prop="weight">
        <el-input v-model.trim="form2.weight" placeholder="未飛重量"></el-input>
      </el-form-item>
      <el-form-item>
        <span v-if="isEnableQuery">
          <el-button type="primary" @click="queryWeight" class="queryBtn">查詢</el-button>
        </span>
        <span v-else>
          <el-button disabled title="您沒有查詢權限" type="primary" @click="queryWeight" class="queryBtn">查詢</el-button>
        </span>
      </el-form-item>
    </el-form>
    <el-divider></el-divider> -->
    ...
    <el-form :inline="true" :model="formInline1" :rules="formInline1Rules" ref="formInline1Ref" class="form-inline1">
      <el-tag style="margin-bottom: 20px;">貨物追蹤列表</el-tag>
      <br>
      ...
      <el-form-item label="搜尋" prop="search">
        <el-input v-model.trim="formInline1.searchValue" placeholder="其它條件"></el-input>
      </el-form-item>
      <el-form-item>
        <span v-if="isEnableQuery">
          <el-button type="primary" @click="queryFlight" class="queryBtn">查詢</el-button>
        </span>
        <span v-else>
          <el-button disabled title="您沒有查詢權限" type="primary" @click="queryFlight" class="queryBtn">查詢</el-button>
        </span>
      </el-form-item>
    </el-form>
    <!-- 分割线 -->
      ...
      <el-table-column label="操作" width="500">
              <template #default="scope">
                ...
                <span v-if="isEnableModify">
                   <el-button type="primary" class="operator" @click="showEditFlightDialogVisible(scope.row)">修改起飛航班</el-button>
                </span>
                <span v-else>
                   <el-button type="primary" class="operator" @click="showEditFlightDialogVisible(scope.row)" disabled title="您沒有修改權限">修改起飛航班</el-button>
                </span>
                <!-- 修改未飛重量按鈕 -->
                <span v-if="isEnableModify">
                   <el-button type="primary" class="operator" @click="showEditWeightDialogVisible(scope.row)">修改未飛重量</el-button>
                </span>
                <span v-else>
                   <el-button type="primary" class="operator" @click="showEditWeightDialogVisible(scope.row)" disabled title="您沒有修改權限">修改未飛重量</el-button>
                </span>
                <!-- 修改航班按鈕 -->
                <!-- <span>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="editLoader">确 定</el-button>
        </span>
      </template>
    </el-dialog>
    <!-- 修改客户 -->
    ...
    >
      ...
      >
        ...
        <el-form-item label="客戶代號" prop="">
          <el-input ref="custRef" v-model.trim="editCustForm.客戶簡稱" placeholder="客户代號" @input="input"></el-input>
        </el-form-item>
        <!-- <el-form-item label="姓名" prop="名稱">
          <el-input v-model.trim="editForm.名稱"></el-input>
        </el-form-item> -->
    ...
    >
      ...
      >
        ...
        <el-form-item label="起飛航班" prop="">
          <el-input ref="flightRef" v-model="editFlightForm.起飛航班" placeholder="如：CI928 / 11, CI6186 / 11, CI6226 / 11" @input="input"></el-input>
        </el-form-item>
        <!-- <el-form-item label="姓名" prop="名稱">
          <el-input v-model.trim="editForm.名稱"></el-input>
        </el-form-item> -->
    ...
    >
      ...
      >
        ...
        <el-form-item label="未飛重量" prop="">
          <el-input ref="weightRef" v-model.trim="editWeightForm.未飛重量" placeholder="未飛重量" @input="input"></el-input>
        </el-form-item>
        <!-- <el-form-item label="姓名" prop="名稱">
          <el-input v-model.trim="editForm.名稱"></el-input>
        </el-form-item> -->
...
export default {
  ...
  computed: {
    ...
    frmTracerDataList () {
      const dataList = []
      let fligntInfo = ''
      for (const item of this.tracerDataList) {
        // 遍历起飞资讯
        fligntInfo = ''
        if (item.起飛資訊.length >= 1) {
          for (const info of item.起飛資訊) {
            fligntInfo += info.班機.replace(' ', '') + ' '
          }
        } else {
          fligntInfo = ''
        }
    ...
    // 分页total
    total () {
      return this.frmTracerDataList.length
    },
    // 分页列表
    pageList: {
      get () {
        return this.frmTracerDataList.slice((this.currentPage - 1) * this.pageSize, this.currentPage * this.pageSize)
      }
    },
    // 主單號類型對應航空公司
    airOptions () {
  ...
  methods: {
    ...
    async editWeight () {
      await this.$refs.editWeightFormRef.validate(valid => {
        ...
        console.log(this.editWeightForm.未飛重量)
        if (this.editWeightForm.未飛重量.trim() === '') return this.$message.error('不得為空，請重新輸入')
        if (isNaN(this.editWeightForm.未飛重量.trim())) return this.$message.error('必須是數字，請重新輸入')
    ...
    showEditWeightDialogVisible (row) {
      ...
      this.editWeightForm.未飛重量 = row.未飛重量
      // this.getBookingData()
      // ajax.get('/api/User', {
      //   params: {
      //     account: row
    ...
    renderPermission () {
      for (const item of this.currentUserPermission) {
        ...
        if (item.MenuUID === parseInt(this.$refs.tracerPageRef.id)) {
    ...
    queryFlightInfo () {
      this.$refs.form1Ref.validate(valid => {
        console.log('form1 mawbno: ', this.form1.mawbNo)
        if (!valid) return false
        // 起飛資訊請求
        this.loading = true
        ajax.post('/api/Operate/track_flight',
          {
            mawbNo: this.form1.mawbNo,
            flight: this.form1.flight
          },
          {
            headers: {
              Authorization: 'Bearer ' + this.token
          ...
          // todo
          if (res === 'OK') this.$message.success('追踪成功')
          setTimeout(() => {
            this.queryFlight()
          }, 3000)
        }).catch(err => {
          ...
          setTimeout(() => {
            this.$message.error('查詢失敗，請再重試一次')
          }, 3000)
        })
      })
    },
    ...
    async editFlight () {
      await this.$refs.editFlightFormRef.validate(valid => {
        console.log(this.editFlightForm.起飛航班)
        if (this.editFlightForm.起飛航班.trim() === '') return this.$message.error('不得為空，請重新輸入')
        if (!valid) return
        if (!this.checkFlightInfo(this.editFlightForm.起飛航班)) {
        ...
        console.log(
          {
            ...
            flight: this.editFlightForm.起飛航班
            // flightDate: this.editFlightForm.預排日期,
            // flightNo: this.editFlightForm.預排航班
          }
        )
    ...
    showEditFlightDialogVisible (row) {
      ...
      // this.editFlightForm.起飛航班 = row.起飛航班
    ...
    async removeMainSheetByNum (num) {
      ...
      const confirmResult = await this.$confirm('此操作將永久取消該主號, 是否繼續?', '提示', {
    ...
    queryFlight () {
      ...
      this.$refs.formInline1Ref.validate(valid => {
        ...
        ajax.get('/api/Operate/track', {
          ...
          this.tracerDataList = res.data
          console.log('tracer data: ', this.tracerDataList)
        }).catch(err => {
          console.log('tracer err: ', err)
          this.loading = false
          this.$message.error('查詢貨物訊息失敗，請稍後重試')
        })
      })
    }
  },
